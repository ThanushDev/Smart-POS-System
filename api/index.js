...
const User = mongoose.models.User || mongoose.model('User', new mongoose.Schema({
  name: String, email: { type: String, unique: true }, password: { type: String }, role: { type: String, default: 'Staff' }, businessId: String
}, { timestamps: true }));
...
app.get('/api/users', async (req, res) => {
  await connectDB();
  try {
    const users = await User.find({ businessId: req.query.businessId });
    res.json(users);
...
app.post('/api/invoices', async (req, res) => {
  ...
  try {
    ...
    for (let item of cart) {
      if (item._id) { await Product.findByIdAndUpdate(item._id, { $inc: { qty: -Number(item.quantity || 0) } }); }
    }
