...
import { Search, Receipt, ShoppingCart, ArrowLeft } from 'lucide-react';
...
const NewBill = () => {
      ...
      if (document.activeElement === searchInputRef.current) {
        ...
        if (e.key === 'Enter') {
          ...
          if (filteredList[selectedIndex]) {
            addToCart(filteredList[selectedIndex]);
          }
        }
  ...
  return (
    <div className="flex h-screen bg-slate-100 font-sans italic overflow-hidden">
      <Sidebar />

      {/* ===== DESKTOP LAYOUT ===== */}
      <main className="hidden md:flex flex-1 p-6 gap-6 overflow-hidden">
        <div className="flex-[1.5] flex flex-col">
          ...
          <div className="space-y-2 overflow-y-auto">
            {searchTerm && filtered.map((p, i) => {
              ...
              return (
                <div key={p._id} onClick={() => addToCart(p)} className={`p-5 rounded-3xl flex justify-between items-center transition-all cursor-pointer ${selectedIndex === i && cartIndex === -1 ? 'bg-indigo-600 text-white shadow-xl scale-[1.02]' : 'bg-white text-slate-600'}`}>
                  <div>
      ...
      {/* ===== MOBILE LAYOUT ===== */}
      <main className="flex md:hidden flex-col flex-1 overflow-hidden pt-16">
        {/* Mobile Tab Switch */}
        <div className="flex bg-white border-b border-slate-100 px-4 py-2 gap-2">
          <button
            onClick={() => setMobileView('search')}
            className={`flex-1 py-2 rounded-xl font-black text-xs uppercase transition-all ${mobileView === 'search' ? 'bg-indigo-600 text-white' : 'text-slate-400 bg-slate-50'}`}
          >
            <Search size={14} className="inline mr-1" />Products
          </button>
          <button
            onClick={() => setMobileView('cart')}
            className={`flex-1 py-2 rounded-xl font-black text-xs uppercase transition-all relative ${mobileView === 'cart' ? 'bg-indigo-600 text-white' : 'text-slate-400 bg-slate-50'}`}
          >
        ...
        {mobileView === 'search' && (
          <div className="flex flex-col flex-1 overflow-hidden p-4">
            <div className="bg-white p-4 rounded-2xl shadow-sm flex items-center gap-3 mb-3 border-2 border-indigo-500">
              <Search className="text-indigo-600 flex-shrink-0" size={20} />
              <input
                ref={searchInputRef}
                type="text"
                placeholder="SEARCH PRODUCT..."
                className="flex-1 outline-none font-black text-base uppercase"
                value={searchTerm}
            ...
            <div className="space-y-2 overflow-y-auto flex-1">
              {searchTerm && filtered.map((p, i) => {
                ...
                return (
                  <div
                    key={p._id}
                    onClick={() => { addToCart(p); toast.success(`${p.name} added!`, { autoClose: 1000 }); }}
                    className="p-4 rounded-2xl flex justify-between items-center bg-white shadow-sm active:bg-indigo-50 transition-all cursor-pointer"
                  >
                    <div>
                      <h4 className="font-black text-sm uppercase">{p.name}</h4>
                      <p className="text-[10px] font-bold text-slate-400">Stock: {p.qty}</p>
                      {hasDiscount && (
                    ...
                    <div className="text-right">
                      {hasDiscount && <p className="text-[10px] line-through text-slate-300">Rs.{p.price}</p>}
                      <p className="font-black text-base text-indigo-600">Rs.{(hasDiscount ? discountedPrice : p.price).toLocaleString()}</p>
                    </div>
                  </div>
                );
              })}
              {searchTerm && filtered.length === 0 && (
                <div className="text-center py-10 text-slate-400 font-bold text-sm">No products found</div>
              )}
              {!searchTerm && (
                <div className="text-center py-10 text-slate-300 font-bold text-sm uppercase">Type to search products</div>
              )}
            </div>
          </div>
        )}
        ...
        {mobileView === 'cart' && (
          <div className="flex flex-col flex-1 overflow-hidden">
            <div className="flex-1 overflow-y-auto p-4 space-y-3">
              {cart.length === 0 && (
                <div className="text-center py-16 text-slate-300 font-bold text-sm uppercase">Cart is empty</div>
              )}
              {cart.map((item, i) => (
                <div key={item._id} className="flex items-center justify-between p-4 rounded-2xl bg-white shadow-sm border border-slate-100">
                  <div className="flex-1">
                    <h5 className="font-black text-xs uppercase">{item.name}</h5>
                    <p className="text-indigo-600 font-black text-xs mt-1">Rs.{(Number(item?.price) || 0).toLocaleString()}</p>
                  </div>
                  <div className="flex items-center gap-3">
                    <button
                      onClick={() => setCart(prev => prev.map((it, idx) => idx === i && it.quantity > 1 ? {...it, quantity: it.quantity - 1} : it))}
                      className="w-8 h-8 rounded-full bg-slate-100 font-black text-slate-600 flex items-center justify-center"
                    >-</button>
                    <span className="font-black text-sm w-6 text-center">{item.quantity}</span>
                    <button
                      onClick={() => setCart(prev => prev.map((it, idx) => idx === i ? {...it, quantity: it.quantity + 1} : it))}
                      className="w-8 h-8 rounded-full bg-indigo-600 text-white font-black flex items-center justify-center"
                    >+</button>
                    <button
                      onClick={() => setCart(prev => prev.filter((_, idx) => idx !== i))}
                      className="w-8 h-8 rounded-full bg-rose-50 text-rose-500 font-black flex items-center justify-center ml-1"
                    >×</button>
                  </div>
                </div>
              ))}
            ...
            <div className="p-4 bg-white border-t-2 border-slate-100">
              <div className="flex justify-between font-black text-xl uppercase text-indigo-600 mb-3">
                <span>Total</span>
                <span>Rs.{(Number(finalTotal) || 0).toLocaleString(undefined, { minimumFractionDigits: 2 })}</span>
              </div>
              <button
                ...
                className="w-full py-4 bg-indigo-600 text-white rounded-2xl font-black uppercase shadow-xl disabled:bg-slate-400"
              >
                {isProcessing ? "PROCESSING..." : "FINISH BILL"}
              </button>
            </div>
          </div>
        )}
      ...
      {/* Payment Modal - same for both */}
      {showPaymentModal && (
        <div className="fixed inset-0 bg-slate-900/90 backdrop-blur-sm flex items-center justify-center z-[999] p-4">
          <div className="bg-white p-8 md:p-12 rounded-[3rem] md:rounded-[4rem] text-center shadow-2xl border-4 md:border-8 border-indigo-600 w-full max-w-[400px]">
            <h2 className="text-2xl md:text-3xl font-black uppercase mb-8 md:mb-10 italic">Pay via <span className="text-indigo-600">{paymentMethod}</span></h2>
            <div className="grid grid-cols-2 gap-4 md:gap-6 mb-8 md:mb-10">
              <div onClick={() => setPaymentMethod('CASH')} className={`p-6 md:p-10 rounded-[2rem] md:rounded-[3rem] border-4 cursor-pointer transition-all ${paymentMethod === 'CASH' ? 'bg-indigo-600 text-white border-indigo-600 scale-105' : 'bg-slate-50 text-slate-300 border-slate-100'}`}>
                <Receipt size={36} className="mx-auto mb-2" />
                <p className="font-black text-lg">CASH</p>
              </div>
              <div onClick={() => setPaymentMethod('CARD')} className={`p-6 md:p-10 rounded-[2rem] md:rounded-[3rem] border-4 cursor-pointer transition-all ${paymentMethod === 'CARD' ? 'bg-indigo-600 text-white border-indigo-600 scale-105' : 'bg-slate-50 text-slate-300 border-slate-100'}`}>
                <Receipt size={36} className="mx-auto mb-2" />
                <p className="font-black text-lg">CARD</p>
              </div>
            </div>
            <button
              onClick={() => processFinalOrder(cart, paymentMethod, finalTotal)}
              disabled={isProcessing}
              className="w-full bg-indigo-600 text-white p-4 rounded-2xl font-black uppercase text-base shadow-xl disabled:opacity-60"
            >
              {isProcessing ? "SAVING..." : "CONFIRM & PRINT"}
            </button>
            <button onClick={() => setShowPaymentModal(false)} className="mt-3 w-full py-3 font-black text-slate-400 text-sm uppercase">CANCEL</button>
          </div>
        </div>
      )}
