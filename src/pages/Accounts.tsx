...
import { UserPlus, Shield, Edit3, User, Eye, EyeOff, X, Search } from 'lucide-react';
...
const Accounts = () => {
  ...
  const fetchUsers = async () => {
    try {
      // මෙතන businessId එක query එකක් විදියට යවනවා
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    try {
      // අලුත් User කෙනෙක් හදද්දිත් මේ admin ගේ businessId එකම දානවා
  ...
  return (
    <div className="flex min-h-screen bg-slate-50 font-sans">
      ...
      <main className="flex-1 p-8 flex flex-col lg:flex-row gap-8">
        ...
        <div className="flex-1">
          <header className="mb-8 flex justify-between items-end">
            <div>
              ...
              <p className="text-slate-400 text-[10px] font-bold uppercase mt-1">ID: {currentUser.businessId}</p>
            </div>
        ...
        {/* Form Area */}
        <div className="w-full lg:w-[400px] bg-white rounded-[3rem] shadow-xl border border-slate-100 flex flex-col h-fit sticky top-8">
          ...
          <form onSubmit={handleSubmit} className="p-8 space-y-5">
            <input type="text" placeholder="Name" className="w-full p-4 bg-slate-50 rounded-2xl outline-none font-bold text-sm focus:ring-2 ring-indigo-50" value={newUser.name} onChange={(e) => setNewUser({...newUser, name: e.target.value})} required />
            <input type="email" placeholder="Email" className="w-full p-4 bg-slate-50 rounded-2xl outline-none font-bold text-sm focus:ring-2 ring-indigo-50" value={newUser.email} onChange={(e) => setNewUser({...newUser, email: e.target.value})} required />
            
            <div className="relative">
              <input type={showPassword ? "text" : "password"} placeholder="Password" className="w-full p-4 bg-slate-50 rounded-2xl outline-none font-bold text-sm focus:ring-2 ring-indigo-50" value={newUser.password} onChange={(e) => setNewUser({...newUser, password: e.target.value})} required />
            ...
            <select className="w-full p-4 bg-slate-50 rounded-2xl outline-none font-bold text-sm appearance-none cursor-pointer" value={newUser.role} onChange={(e) => setNewUser({...newUser, role: e.target.value})}>
              <option value="Staff">Staff Member</option>
              <option value="Admin">Co-Admin</option>
            </select>
            
            <button type="submit" className="w-full py-5 bg-indigo-600 text-white rounded-2xl font-black uppercase tracking-widest shadow-lg shadow-indigo-100 hover:bg-indigo-700 transition-all mt-4">
